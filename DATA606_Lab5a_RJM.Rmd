...
The distribution is unimodal with a very slight right skew.
...
The distribution is still unimodal but the skew is a bit less probably a result of a smaller set. There are also some possible outliers.
...
If the seed is not set or set at a different point, the mean is slightly varied for both samples but lies closely to the population mean in both cases. If the seed is different or not set then the mean is slightly varied.
The mean moved a bit further from the popuation mean for a random sample of 100 but then fell closer to the population mean at size 1000. 
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    means?
There are 5,000 elements in sample_means50 with each element having 50 samples. The distribution looks normal and the mean is very close to 1,500 which in turn is very close to the population mean of 1,499.69. The distribution will still be normal if we increase the number of elements to 50,000 as we are increasing the number of our samples. Also, the mean should still be close to the population mean of 1,499.69.
...
sample_means_small
```
There are 100 elements in this object
...
```{r}
xlimits_10 <- range(sample_means10)
xlimits_50 <- range(sample_means50)
xlimits_100 <- range(sample_means100)

xlimits_10[2] - xlimits_10[1]
xlimits_50[2] - xlimits_50[1]
xlimits_100[2] - xlimits_100[1]

```

From the above, the centre is getting closer to the population with an increasing sample size making the spread smaller.
...
samp_p_m <- mean(samp_p)
samp_p_m

# The best estimate of population mean is $175,032.10 based on a random sample 
# of 50.
```
...
```
    ...
    From the above histogram, we can see that the sampling distribution is normally distributed with a mean centred around $180,000.
    
```{r}
pop_m <- mean(sample_means50)
pop_m
```
    
The population mean is very close to the mean of the 5000 sample means of 5000 random samples of the population of size 50.
...
From the above, we can observe that the means have converged around $180,000 and the spread has become smaller as the sample size has been increased from 50 to 150.
...
A distribution with smaller spread is preferred as there is less standard deviation from the mean which means that more of the observed values are closer to the mean.
