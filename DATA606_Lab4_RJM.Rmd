...
hist(fdims$hgt)
```
Both the male and female height histograms depict normal distribution. However, female height histogram seems to be a bit more right skewed. The male height histogram has a perfect normal distribution. The average heights for males seem to be taller as majority of datapoints are concentrated between 165 to 190 cms whereas female heights are concentrated between 155 to 180 cms.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
The plot seems to depict a nearly normal distribution with a slight skew to the right for the majority of data points.
...
qqline(sim_norm)
```
The plot with the simulated data points yield similar results as previous one with most of the data points on the line with slight deviation at the both ends.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    Yes, these plots present similar results as the initial plot showing that the data points are normally distributed.
...
```
Yes, the female weights appear to come from a normal distribution.
...
```
The closeness between the two probabilities shows that nearly 49% of females will be above the height of 165 cms.
```{r}
summary(fdims$wgt)
```
...
```
Again, 86% of females will have weights greather than 50 kgs.
* * *
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    The stepwise pattern could be a result of data points with similar values or a discrete nature of a particular variable. Age would be discrete whereas the elbow diameter must have a lot of repeated or similar values.
...
qqline(fdims$kne.di)
```
Looking at the plots, the data seems to be slightly right skewed.
