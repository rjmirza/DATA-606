...
A streak length of 1 means Kobe misses a shot after one hit. A streak length of 0 would mean Kobe missing another shot after the first miss.
...
Majority of the streaks either had a length of 0 or 1. Kobe's longest streak was 4 in length before missing.
...
# There are 55 heads in this toss simulation.
```
...
```
The distribution represents the majority of streaks of length 0 or 1. The longest streak was 6.
...
```{r}
sim_basket_2 <- sample(outcomes, size = 133, replace = T, prob = (c(0.45, 0.55)))

sim_streak_2 <- calc_streak(sim_basket_2)
barplot(table(sim_streak_2))

```
The streak distribution came out to be similar with majority of the streaks between 0 and 1 as expected. It is because the percentage for the hits and misses is same as the previous distriution.
...
Both of the distributions have majority of the streaks between 0 and 1. This indicates that there is not enough evidence to ascertain the the hot hand model fits Kobe's shooting patterns.
